...
class TapTester(object):
    ...
    def listen(self):
        ...
        if amplitude > self.tap_threshold:
            ...
            # quiet block.
            self.stream.write(b''.join(self.frames))
            self.frames = []
    ...
    def listenCommand(self,callbackFunction,minSizeKB=1000):
        try:
            block = self.stream.read(INPUT_FRAMES_PER_BLOCK)
        except IOError as e:
            # dammit. 
        ...
        if amplitude > self.tap_threshold:
            ...
            # quiet block.
            dataWav = b''.join(self.frames)
            if(len(dataWav)>minSizeKB):
    ...
    def record(self,label,count,minSizeKB=1000):
        try:
            block = self.stream.read(INPUT_FRAMES_PER_BLOCK)
        except IOError as e:
            # dammit. 
